...
from model.model_ViTriLoss_random import VITriLoss
...
def hard_maximal(train_dataloader, dataset, model, method=opt.center_method, K=opt.K):
  ...
  with torch.no_grad():
    ...
    for obj_id, examples in hard_examples.items():
        examples_len = len(examples)  # 获取每个obj_id对应的examples的长度
        # 随机选择K个不同的索引
        random_indices = random.sample(range(examples_len), K) if examples_len >= K else random.sample(range(examples_len), examples_len)
        # 根据随机选择的索引获取im_id
        hard_examples[obj_id] = [examples[i][1] for i in random_indices]
  ...
  return hard_examples, label_features
...
if __name__ == '__main__':
  ...
  for epoch in tqdm(range(opt.epoch)):
    ...
    #--------------------------------
    hard_examples, label_features = hard_maximal(train_dataloader, dataset, model_image, method=opt.center_method, K=opt.K)
    # center_feature = center_feature.to(torch.float32)
    label_features =  label_features.to(torch.float32)
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        image_embedding = model_image(shuffled_images)
        text_embedding = model_text(shuffled_texts)
        logit_scale = model.logit_scale.exp()
        logits_per_image, logits_per_text = create_logits(image_embedding, text_embedding, logit_scale)

        ground_truth = torch.arange(len(shuffled_images),dtype=torch.long,device=device)
        ...
        if not selected_indices:  # 如果当前批次没有所需的图像，跳过此批次
          ...
          vi_loss = VITriLoss(image_features, selected_obj_id, label_features, device)
