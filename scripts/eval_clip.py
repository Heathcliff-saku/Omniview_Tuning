...
if __name__ == '__main__':
    ...
    device = f"cuda:1" if torch.cuda.is_available() else "cpu" # If using GPU then use mixed precision training.
    # openai clip
    ...
    model = torch.load('/data1/yinpeng.dong/Omniview-Tuning/src/model_checkpoint/ovt_clip_vitb16_epoch=9.pt').to(device)
    ...
    for pair in opt.test_data_label_path:
        ...
        top1, top5 = eval_imagenet_zero_shot_cls(model, device, preprocess, dataset_path, label_path, gt_label_path, tokenizer)