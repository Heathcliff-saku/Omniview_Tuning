...
def hard_maximal(train_dataloader, dataset, model, method=opt.center_method, K=opt.K):
  ...
  with torch.no_grad():
    feature_dim = 768 if opt.clip_model_name == 'ViT-L/14' or opt.clip_model_name == 'ViT-L/14@336px' else 512
...
if __name__ == '__main__':
  ...
  for pair in opt.eval_data_label_path:
    ...
    top1, top5 = eval_imagenet_zero_shot_cls(model, device, preprocess, dataset_path=dataset_path, label_path=label_path, gt_label_path=gt_label_path)
    top1s.append(top1)
    top5s.append(top5)
    torch.cuda.empty_cache()
  ...
  for epoch in tqdm(range(opt.epoch)):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        #       'loss': total_loss,
        #       # 'model': model
        #       }, f"../src/model_checkpoint/{opt.exp}_epoch={epoch}_iter={iter_num}.pt") #just change to your preferred folder/filename
        # iter_num += 1
    lr_scheduler.step()
    ...
    torch.save(model, f"../src/model_checkpoint/{opt.exp}_epoch={epoch}.pt") #just change to your preferred folder/filename
