...
def hard_maximal_hash(center_feature: torch.Tensor, train_dataloader: DataLoader, model, K: int) -> dict:
  ...
  with torch.no_grad():
    ...
    for batch_idx, batch in enumerate(train_dataloader):
      ...
      similarities = torch.norm(image_features-center_feature[object_id], dim=1)
    ...
    for obj_id, examples in hard_examples.items():
      examples.sort(key=lambda x: x[0], reverse=True)  # 根据l2距离降序排列
  ...
  for epoch in tqdm(range(EPOCH)):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        if not selected_indices:  # 如果当前批次没有所需的图像，跳过此批次
          ...
          similarities = torch.norm(image_features-center_feature[selected_obj_id], dim=1)
          vi_loss = similarities.mean()
